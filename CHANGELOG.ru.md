...
Все заметные изменения спецификации X-Alloy Discovery фиксируются в этом файле.

## 0.2.0 — Draft

- Docker stdout/stderr собираются по умолчанию для всех контейнеров.
- `x-alloy-discovery.logs.enabled=false` используется как явный opt-out.
- `x-alloy-discovery.logs.profile` теперь обозначает имя заранее определённого и версионированного pipeline-профиля.
...
- Задокументировано исключение домена `logs` из требования `x-alloy-discovery.enabled`.
...
- Определён контракт переменных окружения `XAD_*` для deployment-специфичных параметров референсной конфигурации.
