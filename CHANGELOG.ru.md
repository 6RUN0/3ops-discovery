...
- Исправлен §14 про мультисеть: контейнер в нескольких Docker-сетях не даёт дублирующихся targets — дефолт `discovery.docker` `match_first_network = true` выбирает первую сеть по алфавиту имён; реальный риск — недостижимость выбранной сети для Alloy (`up == 0`), логи от достижимости не зависят. Дедупликация логов по container ID закреплена e2e-гейтом: контейнер с несколькими exposed-портами читается ровно один раз.
