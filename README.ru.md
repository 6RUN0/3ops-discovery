...
| Путь | Назначение |
|---|---|
| [`docs/manifest.ru.md`](docs/manifest.ru.md) | Нормативный контракт (русскоязычный). Единственный источник истины: имена меток, домены, профили, secret-контракт, требования безопасности, пин версии Alloy. |
...
| [`tools/materialize.py`](tools/materialize.py) | Сборка каталога конфигурации (base ∪ optional) для `alloy_check` и e2e. |
...
Образ Alloy запинен **манифестом** (раздел [14](docs/manifest.ru.md#14-reference-implementation)); `noxfile.ALLOY_IMAGE`
обязан совпадать — это проверяет статический тест.

## Релиз

`uv run nox -s release` собирает `dist/3ops-discovery-<версия>.tar.gz` и
`SHA256SUMS`: контракт, референсная конфигурация, overlay-файлы и
deployment-README. Версия берётся из шапки манифеста — единственного
места, где она записана (в `pyproject.toml` намеренно `0.0.0`).
Содержимое перечисляет `git ls-files`, поэтому неотслеживаемый мусор в
архив не попадает, а сборка воспроизводима: повторный запуск даёт тот же
sha256. Публикация — workflow по тегу `vX.Y.Z`, который обязан совпадать
с версией манифеста.

## Как это держится вместе

Статические гейты извлекают нормативные факты из [`docs/manifest.ru.md`](docs/manifest.ru.md)
...
[`tests/static/asymmetries.py`](tests/static/asymmetries.py). Контрактные значения (интервалы скрейпа,
