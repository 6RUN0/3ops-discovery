...
- for the metrics and database domains SSRF is excluded: the scrape address is built only from `__meta_docker_network_ip` and a private port of the container itself (for database, from the secret file, which labels do not choose beyond `secret-id`), so labels cannot redirect collection to another host;
