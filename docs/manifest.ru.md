...
Обязательный флаг участия контейнера в discovery для доменов metrics, database и blackbox: без `enabled == true` контейнер не попадает в pipeline этих доменов.

Объявление и чтение — разные вещи, и совпадают они не везде. Для доменов otel и ipmi label объявляется по тому же правилу, но ни референс, ни какая-либо реализация его не читает: у otel приёмник OTLP общий и статичный и не может соотнести push с объявившим контейнером (раздел [10.5](#105-domain-otel)), а у ipmi доменного pipeline на стороне Alloy нет вовсе (раздел [10.7](#107-domain-ipmi)). Для этих двух доменов `enabled` — инвентарное объявление намерения, а не управляющий флаг; требовать от реализации его чтения контракт не может.
...
Логовый путь добавляет к этому набору `service_name`, `log_profile` и `source`; полный перечень допустимых stream labels — раздел [6.2](#62-loki-label-cardinality), который нормирует кардинальность, а не состав. Единого места с перечнем эмитируемых labels нет по построению: метрики и логи имеют разные наборы, и раздел [6.2](#62-loki-label-cardinality) — нормативный источник для логов.
...
Label `instance` относится к метрикам; для логов его аналогом служит `service_name` (см. раздел [10.3.5](#1035-labels)). Аналогия неполная и на неё нельзя опираться как на правило: `service_name` выводится только из `logs.service`, тогда как глобальный `instance` на логи не распространяется.

Идентичность серий по доменам. Dashboards и alert rules опираются именно на эти значения, поэтому они часть контракта, а не деталь реализации:

| Домен | `job` | `instance` | Дополнительно |
|---|---|---|---|
| metrics | из `metrics.job` (обязателен) | адрес target; переопределяется `instance` | — |
| database | тип СУБД (`database.type`) | `database.instance`, иначе глобальный `instance`, иначе `secret-id` | — |
| blackbox | константа `blackbox` | адрес пробы (`blackbox.address` или адрес контейнера) | `module` — тип пробы |
| snmp | константа `snmp` | адрес устройства | `device` — имя устройства; `snmp_profile` — имя профиля раздела [8.5](#85-snmp-профили) |

Домен blackbox — задокументированное исключение из правила раздела [5](#5-глобальные-labels) о глобальном `instance`: для пробы instance'ом является сама цель пробы, поэтому глобальное значение здесь не применяется и не переопределяет адрес. Объявленный `instance` у контейнера с blackbox-пробой относится к его собственным метрикам, а не к пробам.
...
Label содержит только major и minor: patch-изменения по определению не меняют семантику labels и в контракте не отражаются. Формат — `<major>.<minor>`, обе части десятичные без ведущих нулей: `^\d+\.\d+$`. Значения `0.2.0`, `v0.2` и `0.2-rc1` контракту не соответствуют; версия спецификации в шапке этого документа задаётся полным SemVer и с этим label не совпадает по формату намеренно — она версионирует текст, а label версионирует смысл labels.

Поведение при неподдерживаемой версии в `0.2` не определено, потому что проверка зарезервирована: реализация обязана принимать любое значение и не имеет права отбрасывать по нему target. Когда проверка появится, она обязана подчиняться правилу домена logs (раздел [10.3.4](#1034-неизвестный-профиль)): логи не теряются ни при какой версии, а отбрасывание по версии допустимо только для доменов с fail-closed-семантикой.
...
Извлекаемые поля. Набор полей, попадающих в structured metadata, — часть определения профиля, а не деталь реализации: обещание версионирования из раздела [8.1](#81-общие-правила) означает, что две независимые реализации одного имени профиля дают одинаковые метаданные. Иначе `generic-json-v1` у одного вендора и у другого — разные вещи под одним именем.

| Профиль | Поля в structured metadata |
|---|---|
| `raw-v1` | — |
| `generic-json-v1` | `level`, `status`, `trace_id`, `request_id` |
| `generic-logfmt-v1` | `level`, `status`, `trace_id`, `request_id` |
| `mixed-v1` | `level`, `status`, `trace_id`, `request_id` (обе ветви) |
| `java-stacktrace-v1` | — |
| `python-stacktrace-v1` | — |

Наборы `generic-json-v1` и `generic-logfmt-v1` совпадают намеренно: профили отличаются форматом входа, а не составом извлекаемого. Отсутствующее в строке поле просто не появляется — это не ошибка разбора.

Предпосылка multiline-профилей. `java-stacktrace-v1` и `python-stacktrace-v1` опознают начало записи по префиксу времени в формате ISO-8601 (`2026-07-27T12:00:00` или с пробелом вместо `T`) в начале строки. Приложение, не печатающее такой префикс, не даёт НИ ОДНОЙ строки, опознанной как начало: весь поток склеивается в блоки по `max_lines` (128), границы которых к содержимому отношения не имеют. Это ограничение профиля, а не сбой: приложению без ISO-8601-префикса подходит `raw-v1` или специализированный профиль с собственным якорем. Raw-fallback раздела [10.3.3](#1033-raw-fallback) здесь не помогает — он рассчитан на ошибку парсера, а склейка ошибкой не является.
...
Database-профиль управляет объёмом сбора экспортёра — какие коллекторы опрашивают СУБД, — а не каденцией скрейпа: scrape-профили раздела [8.2](#82-scrape-профили) относятся только к доменам `metrics` и `blackbox`. Каденция домена database фиксирована и профилем не выбирается: `interval: 60s`, `timeout: 10s`. Величина задана контрактом и в конфигурации указывается явно — наблюдаемое потребителем значение не может быть дефолтом стороннего продукта, который сдвинется при его обновлении. Нагрузка монотонна: basic ≤ standard ≤ extended. `standard-v1` соответствует набору коллекторов экспортёра по умолчанию; несовместимое изменение набора требует нового имени профиля (раздел [8.1](#81-общие-правила)). Значение вне allowlist отбрасывает цель целиком (fail-closed) — как в доменах metrics и blackbox. Домен `logs` — единственное исключение и единственный домен, где неизвестный профиль цель не отбрасывает: логи не теряются, применяется `raw-v1` (раздел [10.3.4](#1034-неизвестный-профиль)). Правило fail-closed действует для всех доменов, кроме logs.
...
Отсутствующий файл секрета: радиус поражения. `secret-id` приходит из Docker-label, то есть из недоверенного входа, поэтому важно, насколько далеко распространяется отказ при ссылке на несуществующий файл. Он ограничен, но шире одного контейнера: в домене database отказ уносит ВЕСЬ `foreach`-блок соответствующего типа СУБД. Контейнер, объявивший валидный по регексу `secret-id` без файла за ним, роняет вычисление блока целиком, и легитимные базы ТОГО ЖЕ типа перестают собираться вместе с ним; базы других типов, живущие в собственных блоках, не затрагиваются. Отказ обратим: после удаления контейнера блок восстанавливается сам.

Соблазнительное прочтение «`local.file` создаётся на каждую итерацию, значит и отказ поитерационный» неверно — это проверено воспроизведением, а не выведено из устройства `foreach`. Top-level `local.file` (файловый провайдер snmp, раздел [10.6](#106-domain-snmp)) локализации не имеет вовсе: его путь задаёт оператор, и отсутствие файла роняет граф каскадом — поэтому overlay 037 загружается только вместе со своими файлами.

Практическое следствие: право запускать контейнеры на хосте включает право остановить сбор со всех баз своего типа на этом коллекторе. Разграничение по типам — не защита, а побочный эффект структуры конфигурации. Мониторинг здоровья компонентов Alloy обязателен и контрактом не заменяется.

Границы редакции секретов. `is_secret = true` гарантирует редакцию в API Alloy и в экспортах компонентов: значение отдаётся как `(secret)`. Гарантия не распространяется на сообщения об ошибках загрузки конфигурации — при несовпадении типа и содержимого файла Alloy может процитировать фрагмент прочитанного значения (например `host:port` чужой БД) в тексте ошибки. Это поведение Alloy, а не выбор контракта, и изменить его конфигурацией нельзя. Практическое следствие: логи самого коллектора чувствительны и требуют того же обращения, что и секреты, — их не следует отправлять туда, куда не отправляют секреты (см. раздел [13.2](#132-разрешено-хранить)).
...
Отключение сбора — единственная в контракте fail-OPEN проверка, и это осознанно. Опт-аут срабатывает строго на литерале `false`; любое другое значение (`False`, `0`, `no`, опечатка) сбор НЕ отключает, и логи продолжают собираться. Направление отказа выбрано в пользу правила раздела [10.3.4](#1034-неизвестный-профиль): опечатка в label не должна приводить к тихой потере логов, а лишний сбор виден и обратим. Для всех остальных labels действует fail-closed (раздел [11](#11-validation-rules)).
...
Профиль `mixed-v1` разбирает строку так (ветки взаимно исключающие, порядок применения на результат не влияет):

1. JSON parser — к строкам, начинающимся с `{`;
2. logfmt parser — к строкам, содержащим `level=`, `msg=` или `status=` в начале строки или после пробела, И не выглядящим как JSON;
3. остальные строки сохраняются raw.

Взаимное исключение ветвей нормативно, а не оптимизация: строка вида `{"msg":"raised level=high"}` подходит под обе эвристики, и без исключения результат зависел бы от порядка стадий — logfmt перезаписал бы значения, извлечённые JSON-парсером. Требование к якорю (`level=` в начале строки или после пробела, а не в любом месте) отсекает совпадения внутри слов и значений.
...
| `ru.3ops.discovery.blackbox.address` | Нет | Хост проверки без схемы и без порта; по умолчанию адрес контейнера |
...
`blackbox.address` содержит ТОЛЬКО хост (`example.com` или IP): ни схемы, ни порта, ни пути. Цель пробы собирается реализацией из `address`, `scheme`, `port` и `path` по-разному для каждого модуля, поэтому порт, записанный в сам `address`, привёл бы к цели с двумя портами. Формат цели пробы зависит от модуля: HTTP-модули получают URL `scheme://host:port/path`, TCP- и TLS-модули — `host:port`, ICMP-модули — только host. Каждой серии blackbox-проб Alloy добавляет label `module` с именем использованного модуля.

Для ICMP-модулей значение `blackbox.port` в саму пробу не входит (у ICMP нет порта), но label остаётся обязательным и проходит ту же валидацию: объявленный порт должен совпадать с реальным экспонированным портом контейнера, иначе target отбрасывается. Правило действует и при внешнем `blackbox.address`: порт сверяется с приватными портами самого контейнера, поэтому проба с `address: example.com` и `port: 443` из контейнера, не экспонирующего порт 443, молча отбрасывается. Последствия `blackbox.address` для модели доверия — раздел [13.5](#135-доверие-к-labels).
...
Приёмник слушает конвенциональные порты OTLP: `4317` (gRPC) и `4318` (HTTP). Значения зафиксированы контрактом и переменной окружения не параметризуются — в отличие от endpoint'ов раздела [14.1](#141-параметры-окружения), это не deployment-величина, а адрес, который отправители знают заранее: смена порта потребовала бы изменить конфигурацию каждого клиента. Развёртывание, которому нужен другой порт, меняет overlay [`060_otel`](../alloy-optional/060_otel.alloy) — это кастомизация раздела [14.3](#143-кастомизация-референсной-конфигурации), а не параметр.
...
- `enabled == true` — кроме домена logs (см. раздел [10.3.1](#1031-политика-сбора-по-умолчанию)); для доменов otel и ipmi проверка неприменима: их labels декларативны и pipeline по ним не создаётся (раздел [5](#5-глобальные-labels));
...
- `profile` входит в allowlist профилей домена (раздел [8](#8-профили)) — кроме домена logs, где неизвестный профиль не отбрасывает контейнер, а приводит к `raw-v1` (раздел [10.3.4](#1034-неизвестный-профиль));
...
У обязательных labels значения по умолчанию нет, поэтому для них то же правило означает отбрасывание: пустой обязательный label = отсутствующий обязательный label = target отбрасывается (fail-closed). Это не частный случай, а следствие: реализация обязана проверять присутствие непустого значения отдельно, потому что сравнение пустого объявленного порта с пустым метаданным (`keepequal`) проходит успешно и цель проскакивает. Референс реализует это явными keep-правилами присутствия для `metrics.port`, `metrics.job` и `blackbox.port`.
...
Логи самого коллектора — чувствительные данные. Alloy пишет в них диагностику загрузки конфигурации, а она может содержать фрагменты прочитанных значений, включая помеченные `is_secret` (границы редакции — раздел [9](#9-secret-contract)). Референсная конфигурация собирает stdout/stderr всех контейнеров, включая контейнер самого Alloy, — это осознанный выбор в пользу наблюдаемости коллектора, а не недосмотр. Если backend логов доверен меньше, чем каталог секретов, сбор логов коллектора отключают явным `ru.3ops.discovery.logs.enabled=false` на его контейнере.
...
Socket proxy сужает привилегию, но не устраняет её. Allowlist прокси работает по секциям API, а не по отдельным путям: разрешив `/containers`, он разрешает ВСЕ GET-запросы этой секции — включая `/containers/{id}/json` (переменные окружения контейнера, то есть пароли, переданные через `environment`), `/containers/{id}/archive?path=/` (чтение произвольного файла из любого контейнера) и `/containers/{id}/export`. Иначе discovery работать не может: `loki.source.docker` читает логи через ту же секцию. Практический вывод: коллектор с доступом к прокси равнозначен читателю секретов всех контейнеров хоста, и прокси следует держать во внутренней сети, недоступной нагрузкам, — он снижает риск компрометации демона, а не риск чтения чужих данных.

Требование «Alloy не должен быть доступен из недоверенной Docker-сети» выполнимо не полностью, и это свойство модели, а не недосмотр конфигурации. Скрейп контейнера означает общую с ним сеть, поэтому коллектор всегда сосед тех, кого собирает: HTTP-сервер Alloy (`/metrics`, неаутентифицированный `/api/v0/web/components`) и opt-in OTLP-приёмник видны каждому контейнеру этой сети. Разделение сетей проблему не решает — при нескольких сетях `discovery.docker` берёт только первую по алфавиту (раздел [14](#14-reference-implementation)), то есть коллектор всё равно обязан находиться в сети наблюдаемых. Практический вывод: недоверенные нагрузки отделяются не сетью, а отдельным коллектором; в общей сети следует исходить из того, что список компонентов и их аргументы (значения, помеченные `is_secret`, редактируются) доступны соседям.
...
- для доменов metrics и database SSRF исключён двумя условиями сразу, и второе так же обязательно, как первое. Адрес scrape строится только из `__meta_docker_network_ip` и приватного порта самого контейнера (для database — из файла секрета, который labels не выбирают за пределами `secret-id`), поэтому labels не могут перенаправить сбор на другой хост. Ответ scrape — тоже недоверенный вход: по умолчанию `prometheus.scrape` следует редиректам, и контейнер, ответивший `302` на link-local адрес, увёл бы туда следующий запрос от имени коллектора. Поэтому во всех `prometheus.scrape` референса задано `follow_redirects = false`; конфигурация, не выполняющая этого условия, гарантию не даёт;
...
Политика версии Alloy. Тег в команде выше нормативен: контракт проверен именно на нём, и он же обязан стоять в `noxfile.ALLOY_IMAGE` (сверяется статическим гейтом в обе стороны). Диапазон совместимых версий контракт не объявляет — не потому, что версии не важны, а потому что честно проверена ровно одна: обновление тега — изменение контракта и проходит те же гейты (`alloy fmt`, `validate` для всех комбинаций, e2e). Нижняя граница не указана намеренно: `foreach` экспериментален, а у экспериментальных блоков upstream не гарантирует ни стабильности синтаксиса, ни наличия в следующей версии.

Конфигурации без экспериментальных возможностей контракт не предлагает: `foreach` находится в базовом файле [`030_database.alloy`](../alloy/030_database.alloy), поэтому флаг `--stability.level=experimental` обязателен для всего референса, а не только для домена database. Развёртывание, которому экспериментальные возможности недопустимы, исключает файл 030 целиком — вместе с доменом database; остальные домены от `foreach` не зависят.
...
Привилегированные overlay получают deployment-привилегии уровня хоста и наблюдают хост или все его контейнеры целиком, вне discovery-скоупа: [`070_host-metrics`](../alloy-optional/070_host-metrics.alloy) (host procfs/sysfs/rootfs), [`080_host-logs`](../alloy-optional/080_host-logs.alloy) (systemd journal), [`075_container-metrics`](../alloy-optional/075_container-metrics.alloy) (host cgroup namespace + `/sys/fs/cgroup:ro`; host PID namespace не требуется и не выдаётся). Выдача таких привилегий — осознанное deployment-решение, не дефолт. Формулировка «привилегии уровня хоста» конкретна: `070` монтирует корень файловой системы хоста (`/:/rootfs:ro`) ради `node_filesystem_*`, а это чтение ВСЕГО — `/etc/shadow`, приватных ключей SSH, слоёв `/var/lib/docker` вместе с содержимым остальных контейнеров. Смонтировать меньше нельзя: метрики файловых систем требуют видеть точки монтирования. Единственное, что референс маскирует отдельно, — сокет Docker (`/dev/null` поверх `/rootfs/var/run/docker.sock`), чтобы привилегия чтения не превратилась в привилегию управления демоном; всё остальное read-only и доступно. `080` даёт чтение системного журнала, `075` — статистику всех контейнеров хоста, включая не участвующие в discovery.
